...
{
    x_ += dx;
    x_ = x_ > world_->get_width() ? world_->get_width() - 1 : x_;
    x_ = x_ < 0 ? 0 : x_;
    y_ += dy;
    y_ = y_ > world_->get_height() ? world_->get_height() - 1 : y_;
    y_ = y_ < 0 ? 0 : y_;
}

void Robot::draw(SDL_Renderer* renderer, const int window_w, const int window_h, const int cell_size) 
{
